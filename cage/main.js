...
(function main() {
  ...
  bluePlayer
    .on("draw", ({ row, column, color, isMyTurn }) =>
      playerDrawer.draw(row, column, color, isMyTurn),
    )
    .on("score", ({ score, isBottomPlayer }) =>
      scoreDrawer.draw(score, isBottomPlayer),
    )
  ...
  const board = new Board(boardElement, placeWallBtn, redPlayer, bluePlayer);

  board
    .on("change", (state, cb) => boardDrawer.draw(state, cb))
    .on("reset", () => boardDrawer.reset())
    .on("wall-move", (state) => boardDrawer.drawWalls(state))
    .on("track-move", (state) =>
